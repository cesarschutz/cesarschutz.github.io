...
O Java 26 chegou à disponibilidade geral (GA) em 17 de março de 2026 ([JDK 26](https://openjdk.org/projects/jdk/26/)) e o Java 27 em 15 de setembro de 2026 ([JDK 27](https://openjdk.org/projects/jdk/27/)). A próxima LTS prevista é o **Java 29, em setembro de 2027**. É o que diz o [Oracle Java SE Support Roadmap](https://www.oracle.com/java/technologies/java-se-support-roadmap.html) (atualizado em 15/09/2026), que classifica o 26, o 27 e o 28 como não LTS e avisa que essa classificação e as datas ainda podem mudar. Nesse roadmap, cada versão não LTS tem Premier Support só até a versão seguinte sair: setembro de 2026 para o 26 e março de 2027 para o 27. Para o Java 29, a Oracle prevê Premier Support até setembro de 2032; para o Java 25, até setembro de 2030. Outros fornecedores de builds do OpenJDK publicam seus próprios prazos. Na data desta atualização, o OpenJDK ainda não tinha publicado a página do projeto JDK 29.

O post é para quem está no **Java 25** e quer acompanhar o que muda até o 29. O ponto de partida é o [post do Java 25](/posts/java-25/); quem vem do Java 21 deve ler os dois.

> **Preview e incubadora não são definitivos.** Pela [JEP 12](https://openjdk.org/jeps/12), um recurso em **preview** está completo, mas não é permanente: numa versão futura ele pode virar final, com ou sem ajustes, ou ser removido. Para usá-lo é preciso `--enable-preview` na compilação e na execução, e uma JVM não carrega classes compiladas com os previews de outra versão. As APIs em **incubadora** ficam em módulos `jdk.incubator.*`, precisam de `--add-modules` e mudam com a mesma liberdade. Os exemplos de preview abaixo refletem o Java 27 e podem precisar de ajuste no Java 28 ou no 29.

Os exemplos que só dependem do Java 26 foram compilados e executados no Temurin 26.0.2. Os que usam recursos exclusivos do 27 (`Set.ofLazy`, o `StructuredTaskScope` do 27, a redação de dados do JFR e `-XX:AOTMode=required`) seguem as assinaturas do código-fonte do JDK 27 (tag `jdk-27-ga`) e as notas de release.
...
- **Java 27** ([página do projeto](https://openjdk.org/projects/jdk/27/)), 9 JEPs: G1 como padrão em qualquer ambiente, compact object headers por padrão, TLS 1.3 com troca de chaves pós-quântica e redação de dados sensíveis no JFR.
...
Até o Java 25, uma virtual thread que tentava inicializar uma classe que outra thread já estava inicializando ficava **presa (pinned)** à carrier durante a espera. Segundo as [notas de release do Java 26](https://www.oracle.com/java/technologies/javase/26-relnote-issues.html), na maioria dos casos ela agora é desmontada e libera a carrier para outras virtual threads. A mudança também ataca um caso patológico: todas as carriers presas nessa espera enquanto o inicializador depende de uma virtual thread para terminar, o que podia travar o sistema inteiro. Não é uma JEP, e não é preciso mudar nada no código.
...
**O problema.** O `java.net.http.HttpClient` ([JEP 321](https://openjdk.org/jeps/321), Java 11) falava HTTP/1.1 e HTTP/2. O HTTP/3, padronizado pela IETF em 2022, roda sobre QUIC (UDP) em vez de TCP e evita problemas de congestionamento como o head-of-line blocking.

**Como funciona.** O HTTP/3 é **opt-in**: o padrão continua sendo HTTP/2. Você escolhe `HttpClient.Version.HTTP_3` no cliente ou em cada requisição. Como não dá para saber de antemão se o servidor fala HTTP/3, a JEP descreve quatro estratégias de descoberta:
...
public class ClienteHttp3 {
    public static void main(String[] args) throws Exception {
        ...
        // Primeiro fala HTTP/2 ou 1.1; migra para HTTP/3 se o servidor anunciar Alt-Svc
        ...
        IO.println(primeira.version());   // HTTP_2: descobre o Alt-Svc

        var segunda = client.send(request, BodyHandlers.ofString());
        IO.println(segunda.version());    // HTTP_3, se o servidor anunciou
    }
}
```

No Temurin 26.0.2, com acesso à internet, esse programa imprime `HTTP_2` e depois `HTTP_3`.

Limitações da primeira versão, segundo a JEP: só funciona com o provider TLS padrão (SunJSSE), não há API pública de QUIC nem servidor HTTP/3, e o `java.net.URL` continua só em HTTP/1.1.
...
- `Process` implementa `AutoCloseable`: fechar o processo garante que ele terminou e libera streams e recursos.
- `Comparator` ganhou os métodos default `min(T, T)` e `max(T, T)`.
- `UUID.ofEpochMillis(long)` cria UUIDs versão 7, que carregam o timestamp.
...
public class ApisMenores {
    public static void main(String[] args) throws Exception {
        ...
        IO.println(Duration.MAX);
    }
}
```

**Java 27** ([notas de release](https://jdk.java.net/27/release-notes)): `KeyStore.getCreationInstant` devolve a data de criação de uma entrada como `Instant`; na Foreign Function & Memory API, `Linker.Option.captureCallState` também inicializa o estado thread-local antes da downcall; e os formatadores ISO predefinidos de `DateTimeFormatter` passam a aceitar offsets curtos, como `+01`, no parsing.
...
O diagrama resume as duas mudanças de padrão do Java 27, detalhadas a seguir: a escolha do coletor e o tamanho do cabeçalho dos objetos.

### G1 com menos sincronização entre aplicação e GC

**Chegou em:** Java 26 (final, [JEP 522](https://openjdk.org/jeps/522))

O G1 registra referências entre regiões do heap numa **card table**, atualizada por pequenos trechos de código injetados na aplicação (as write barriers). Threads de otimização trabalham nessa tabela em segundo plano e, até o Java 25, precisavam se sincronizar com as threads da aplicação, o que deixava as barreiras complexas e lentas. A JEP 522 cria uma **segunda card table**: a aplicação escreve numa tabela sem sincronizar, os otimizadores trabalham na outra, e o G1 troca as duas de forma atômica quando precisa.

A JEP relata ganhos de throughput de 5% a 15% em aplicações que modificam muitos campos de referência, e de até 5% nas demais. No x64, as write barriers caíram de cerca de 50 instruções para 12. O custo é memória nativa: cada card table ocupa 0,2% da capacidade do heap, ou cerca de 2 MB por GB. Não há flag nova para ligar.

### G1 como coletor padrão em qualquer ambiente

**Chegou em:** Java 27 (final, [JEP 523](https://openjdk.org/jeps/523))

Desde o Java 9 ([JEP 248](https://openjdk.org/jeps/248)), o G1 era o padrão apenas em ambientes "server". Com 1 CPU ou menos de 1792 MB de memória física, a JVM escolhia o Serial. A JEP 523 afirma que o G1 ficou competitivo com o Serial em qualquer tamanho de heap, em parte graças à JEP 522. Por isso, sem um coletor na linha de comando, a JVM agora escolhe **sempre o G1**. O Serial continua disponível com `-XX:+UseSerialGC`.

Na prática, isso atinge principalmente containers com limites baixos, porque a JVM considera os limites do container ao contar CPUs e memória. Se o limite é 1 CPU ou menos de 1792 MB e ninguém escolheu o coletor, o Java 27 passa a rodar G1. As flags `AlwaysActAsServerClassMachine` e `NeverActAsServerClassMachine`, que influenciavam essa escolha, foram depreciadas no Java 26 ([notas de release do 26](https://www.oracle.com/java/technologies/javase/26-relnote-issues.html)).

### Compact object headers ligados por padrão

**Chegou em:** Java 25 (final, [JEP 519](https://openjdk.org/jeps/519)) → Java 27 (final, [JEP 534](https://openjdk.org/jeps/534))

Cada objeto Java tem um cabeçalho. Em arquiteturas de 64 bits ele ocupava 96 bits, e com compact object headers passa a 64. No Java 25 o recurso já era de produto, mas era preciso pedir com `-XX:+UseCompactObjectHeaders`. No 27, esse é o layout padrão. A JEP 534 cita os experimentos que motivaram a mudança, entre eles um cenário em que o SPECjbb2015 usou 22% menos heap e 8% menos CPU, o uso em centenas de serviços em produção na Amazon e a adoção como padrão no SapMachine, a distribuição da SAP.

Para voltar ao layout antigo, use `-XX:-UseCompactObjectHeaders`. As [notas do Java 27](https://jdk.java.net/27/release-notes) avisam que essa flag deve ser depreciada e removida no futuro, e que o JDK traz arquivos CDS separados para cada modo.

### Cache AOT com qualquer coletor, inclusive ZGC

**Chegou em:** Java 26 (final, [JEP 516](https://openjdk.org/jeps/516))

O cache AOT do Project Leyden ([JEP 483](https://openjdk.org/jeps/483), Java 24, e [JEP 514](https://openjdk.org/jeps/514), Java 25) melhora startup e warmup ao guardar classes já carregadas e ligadas numa execução de treino. Até o Java 25, os objetos Java desse cache ficavam num formato ligado ao layout de memória dos coletores Serial, Parallel e G1, incompatível com o ZGC. Na prática, era preciso escolher entre a latência baixa do ZGC e o startup rápido do cache.

A JEP 516 cria um formato **independente de GC**: as referências entre objetos viram índices lógicos, e uma thread em segundo plano materializa os objetos no heap enquanto a aplicação sobe. A JVM escolhe o formato por heurística. Se o treino usou ZGC, `-XX:-UseCompressedOops` ou heap acima de 32 GB, grava no formato streamable. Se usou `-XX:+UseCompressedOops`, grava no formato mapeável de antes. O formato streamable pode ser forçado com `-XX:+AOTStreamableObjects`.
...
No Java 27, `-XX:AOTMode=required` passou a ser um alias de `-XX:AOTMode=on` e é a forma recomendada, porque `on` deve ser depreciado e removido ([notas do 27](https://jdk.java.net/27/release-notes)). No Java 26, a JVM não inicia com `-XX:AOTMode=required`.
...
### JFR redige dados sensíveis antes de gravar

**Chegou em:** Java 27 (final, [JEP 536](https://openjdk.org/jeps/536))

**O problema.** Uma gravação do JDK Flight Recorder guarda os argumentos da linha de comando, as variáveis de ambiente e as system properties iniciais. É útil para diagnóstico, mas senhas passadas por `-D`, tokens em variáveis de ambiente e argumentos como `--dbpassword` iam parar, sem nenhuma proteção, no arquivo `.jfr` que depois é compartilhado ou anexado a um chamado.

**Como funciona.** A partir do Java 27, o JFR **redige por padrão**, ainda dentro do processo, os valores que batem com uma lista de filtros. Para chaves, a lista inclui padrões como `*password*`, `*secret*`, `*token*`, `*auth*` e `*api*key*`. Para argumentos, cobre formas como `-*password *`, que pega a opção e o valor seguinte. O valor redigido aparece como `[REDACTED]`. Dá para acrescentar filtros próprios, carregá-los de arquivo ou desligar os padrões:

```bash title="jfr-redacao.sh"
...
# Filtros em arquivo e log de depuração do que foi redigido
...
**O problema.** Desde o JDK 5, `Field.setAccessible(true)` seguido de `Field.set(...)` altera até campos `final`. Com isso, a JVM não pode confiar que um `final` de instância é realmente constante: isso atrapalha otimizações (como o constant folding das Lazy Constants) e quebra a integridade do programa.

**Como funciona.** No Java 26, alterar um campo `final` por reflexão ainda funciona, mas gera um **aviso** na primeira vez em cada módulo. A JEP prevê que uma versão futura passe a **lançar exceção** por padrão. O comportamento é controlado por `--illegal-final-field-mutation=allow|warn|debug|deny` (`warn` é o padrão no 26), e a permissão é dada explicitamente com `--enable-final-field-mutation=ALL-UNNAMED` ou com uma lista de módulos. `--add-opens` não basta para evitar o aviso.
...
No Java 26 saíram o pacote `java.applet` inteiro, `javax.swing.JApplet`, `java.beans.AppletInitializer` e os membros de `java.beans.Beans` e `javax.swing.RepaintManager` que dependiam deles. Um efeito colateral: `URL.getContent()` e `URLConnection.getContent()` não devolvem mais `java.applet.AudioClip` para conteúdo de áudio. Para tocar áudio, a JEP aponta a `javax.sound.SoundClip`, introduzida no Java 25.
...
| JVM Compiler Interface (JVMCI): módulos `jdk.internal.vm.ci`, `jdk.graal.compiler` e `jdk.graal.compiler.management`, flags `*JVMCI*` e `-XX:+UseGraalJIT` | 27 | [Notas do 27](https://jdk.java.net/27/release-notes) |
...
- **Java 28 (com alvo):** port macOS/x64 depreciado para remoção ([JEP 541](https://openjdk.org/jeps/541)). Segundo a JEP, os engenheiros da Oracle deixam de manter esse port a partir do JDK 27.

## Ainda em preview ou incubadora

Nenhum destes recursos é definitivo: todos exigem `--enable-preview` (ou `--add-modules`, no caso da incubadora) e mudaram de API entre uma versão e outra. Os exemplos refletem o Java 27.
...
**Como funciona.** `StructuredTaskScope` trata um grupo de subtarefas como uma unidade presa a um bloco `try`. A thread dona abre o escopo, dispara as subtarefas com `fork` (por padrão, cada uma numa virtual thread), espera tudo com `join` e fecha o escopo ao sair do bloco. A política de conclusão fica num `Joiner`. Na política padrão, se uma subtarefa falhar, as outras são canceladas. O `close()` sempre espera as threads terminarem, então nenhuma sobra depois do bloco. No thread dump em JSON (`jcmd <pid> Thread.dump_to_file -format=json <arquivo>`), a hierarquia de escopos aparece.
...
Para rodar no Java 27: `java --enable-preview --source 27 Checkout.java`.
...
public class Aplicacao {
    ...
    public static void main(String[] args) {
        ...
        IO.println(CONEXAO.get());      // devolve o mesmo valor
        IO.println(WORKERS.get(2));     // só o índice 2 é criado
        IO.println(TAXAS.get("USD"));   // só USD é carregado
        IO.println(OPCOES.contains(Opcao.DRY_RUN));
    }
}
```

Para rodar no Java 27: `java --enable-preview --source 27 Aplicacao.java`. No Java 26, `Set.ofLazy` não existe; o restante do exemplo funciona com `--source 26`.
...
public class ChavesPem {
    public static void main(String[] args) throws Exception {
        ...
        IO.println(publica.getAlgorithm() + " / " + privada.getAlgorithm());

        // Ou sem saber o tipo de antemão, com pattern matching
        switch (decoder.decode(publicaPem)) {
            case PublicKey pk -> IO.println("chave pública " + pk.getFormat());
...
O `default` é obrigatório no `switch`. `BinaryEncodable` tem pelo menos uma implementação que não é pública, e a JEP explica que isso permite ampliar a lista no futuro sem quebrar código existente.

**O que mudou no caminho.** No Java 26, `PEMRecord` virou `PEM`, os métodos `encryptKey` de `EncryptedPrivateKeyInfo` viraram `encrypt`, e encoder e decoder passaram a cifrar e decifrar `KeyPair` e `PKCS8EncodedKeySpec`. No Java 27, `DEREncodable` virou `BinaryEncodable`, `PEM` deixou de ser record e virou classe comum, `PEMDecoder.withFactory` virou `withFactoriesOf`, e entrou a exceção não verificada `javax.crypto.CryptoException`. A [JEP 542](https://openjdk.org/jeps/542), com alvo no Java 28, propõe finalizar a API **sem outras mudanças**.
...
A Vector API expressa cálculos vetoriais que o JIT compila para instruções SIMD da CPU. Nas duas versões ela voltou sem mudança relevante de API; no 27, só a biblioteca SLEEF usada em ARM e RISC-V foi atualizada. Pelas JEPs, ela fica na incubadora até que os recursos necessários do Project Valhalla cheguem como preview. Em 16/09/2026, o primeiro preview de Value Objects ([JEP 401](https://openjdk.org/jeps/401)), do Valhalla, já tinha alvo no Java 28.
...
Para rodar: `java --add-modules jdk.incubator.vector SomaVetorial.java`. Sem o `--add-modules`, o pacote não fica visível e a compilação falha.
...
1. **Recompilar tudo o que usa preview.** Pela [JEP 12](https://openjdk.org/jeps/12), uma JVM não carrega classes que dependem dos previews de outra versão. E as APIs mudaram entre versões: exceções do `StructuredTaskScope` (`FailedException` → `ExecutionException`), `StableValue` → `LazyConstant` sem `isInitialized`/`orElse`, `DEREncodable` → `BinaryEncodable`. Com a dominância mais rígida em `switch` com primitivos, alguns `switch` que compilavam deixam de compilar.
2. **Avisos de `final` alterado por reflexão** ([JEP 500](https://openjdk.org/jeps/500)). Procure os `WARNING: Final field ...` nos logs e rode os testes com `--illegal-final-field-mutation=deny`. Frameworks de injeção, mocks e serialização são os suspeitos de sempre.
3. **Coletor em containers pequenos** ([JEP 523](https://openjdk.org/jeps/523)). Com 1 CPU ou menos de 1792 MB, o padrão passa de Serial para G1. Meça memória e latência e, se preciso, fixe `-XX:+UseSerialGC`.
4. **Layout de objeto** ([JEP 534](https://openjdk.org/jeps/534)). Os compact object headers vêm ligados. Se algo depender do layout antigo, `-XX:-UseCompactObjectHeaders` desliga.
...
Em desenvolvimento. A [página do JDK 28](https://openjdk.org/projects/jdk/28/) ainda não publicou o cronograma, e o roadmap da Oracle prevê o lançamento para março de 2027. Na última atualização da página (27/08/2026), estas eram as JEPs com alvo no 28; a coluna Situação mostra o status de cada JEP em 16/09/2026.
...
| [541](https://openjdk.org/jeps/541) | Deprecate the macOS/x64 Port for Removal | Depreciação | Integrada ao 28 |
| [542](https://openjdk.org/jeps/542) | PEM Encodings of Cryptographic Objects | Final | Integrada ao 28 |
...
- Anteriores ao Java 26: [JEP 248](https://openjdk.org/jeps/248), [JEP 289](https://openjdk.org/jeps/289), [JEP 321](https://openjdk.org/jeps/321), [JEP 398](https://openjdk.org/jeps/398), [JEP 483](https://openjdk.org/jeps/483), [JEP 470](https://openjdk.org/jeps/470), [JEP 502](https://openjdk.org/jeps/502), [JEP 505](https://openjdk.org/jeps/505), [JEP 507](https://openjdk.org/jeps/507), [JEP 508](https://openjdk.org/jeps/508), [JEP 514](https://openjdk.org/jeps/514), [JEP 519](https://openjdk.org/jeps/519)
...
- Processo: [JEP 12 — Preview Features](https://openjdk.org/jeps/12)
